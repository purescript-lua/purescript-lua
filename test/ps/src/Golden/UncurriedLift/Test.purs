...
-- | effectful @runEffectFn2@ site sits in statement position, where the
-- | run wrapper's thunk (run by magic-do, shed by codegen) fuses away to
...
sumST :: forall r. STFn2 Int Int r Int
sumST = mkSTFn2 \a b -> pure (a + b)
...
main :: Effect Unit
main = do
  -- A non-tail effect statement: the run wrapper's thunk (run by
  -- magic-do, shed by codegen) fuses away, so this becomes a direct
  -- `local _ = logTwice("hello", "world")`.
